package com.example.ParentApp0x5B10x5D;
...
import com.example.ParentApp0x5B10x5D.databinding.ActivityMainBinding;
...
public class ParentApp0x5B10x5D extends AppCompatActivity implements OnFragmentInteractionListener , CvCameraViewListener2 {
 ...
 // properties specific for Camera block
   ...
   private boolean isCamera2Supported() {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        com.example.ParentApp0x5B10x5D.databinding.ActivityMainBinding activityMainBinding = ActivityMainBinding.inflate(LayoutInflater.from(this));
...
mReceiveBlockNames = getResources().getStringArray(R.array.tcpReceiveBlockNames);
    ...
    private ParentApp0x5B10x5D thisClass;
    private final Thread BgThread = new Thread() {
    @Override
    public void run() {
            String argv[] = new String[] {"MainActivity","ParentApp0x5B10x5D"};
           ...
           case "dot1":
              if (hasCamera2Support) {
               ...
               if (checkIfAllPermissionsGranted()) {
                   resumeCamera();
	                System.loadLibrary("ParentApp0x5B10x5D");
                   if (!BgThread.isAlive())
                       BgThread.start();
               }
               break;
...
public void audioFileReadTerminate(int idx) {
    ...
    private native int naMain(String[] argv, ParentApp0x5B10x5D pThis);
